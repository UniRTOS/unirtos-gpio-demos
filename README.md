...
This demo demonstrates the basic development workflow for GPIO control on UniRTOS, and is suitable as a beginner example for digital I/O peripheral debugging and hardware interface development.

- Demonstrates GPIO pin initialization with configurable direction (input/output), pull-up/pull-down resistors, and initial level state
- Demonstrates reading back GPIO pin direction, pull configuration, and current level to verify expected settings
- Provides a reusable configuration table (`g_unir_gpio_cfg`) for batch GPIO initialization
- Easily extensible to button detection, LED control, relay driving, and interrupt-based event handling
...
```bash
unirtos-cli new -r unirtos-gpio-demos
```

To specify a version:

```bash
unirtos-cli new -r unirtos-gpio-demos -v 1.0.0
```

### 3. Enter the Project and Build

```bash
cd unirtos-gpio-demos-1.0.0
