...
本 Demo 展示 UniRTOS 上 GPIO 控制的基础开发流程，适合作为数字 I/O 外设调试与硬件接口开发的入门样例。

- 演示 GPIO 引脚初始化，支持配置方向（输入/输出）、上下拉电阻及初始电平
- 演示回读 GPIO 引脚方向、上下拉配置和当前电平，以验证配置是否符合预期
- 提供可复用的配置表（`g_unir_gpio_cfg`），支持批量初始化多路 GPIO
- 便于扩展为按键检测、LED 控制、继电器驱动及中断事件处理
